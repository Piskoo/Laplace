namespace LaplaceCSharp
{
    ...
    {
        public void processImage(byte[] original, byte[] processed, int[] filter, int begStride, int stride)
        {
            int bpp = 4; // bitsperpixel
            int i = begStride;
            for (int j = 0; j != stride - 2*bpp; j += bpp) // for full stride
            {
                int blue = 0;
                int green = 0;
                int red = 0;
